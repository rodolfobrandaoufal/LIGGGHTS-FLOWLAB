/* ----------------------------------------------------------------------
   Standalone GPU_DEM two-particle Hookean collision regression test.
------------------------------------------------------------------------- */
...
double spring_energy(const CpuState &state, double normal_stiffness)
{
  const double distance = std::fabs(state.x[1] - state.x[0]);
  const double overlap = state.radius[0] + state.radius[1] - distance;
  return overlap > 0.0 ? 0.5 * normal_stiffness * overlap * overlap : 0.0;
}
...
void cpu_velocity_verlet_step(CpuState &state, const HookeNormalParams &params, double dt)
{
...
void gpu_velocity_verlet_step(GpuParticleData &particles, GpuNeighborList &neighbors,
                              const NeighborBuildParams &neighbor_params,
                              const HookeNormalParams &contact_params,
                              GpuDemContext &context, double dt)
{
  const double dtf = 0.5 * dt;
...
int main()
{
  ...
  cpu.v[0] = 1.0;
  cpu.v[1] = -1.0;
  const double initial_energy = kinetic_energy(cpu) + spring_energy(cpu, contact_params.normal_stiffness);
  ...
  for (int step = 0; step < steps; ++step) {
    ...
    gpu_velocity_verlet_step(particles, neighbors, neighbor_params, contact_params, context, dt);
  }
  ...
  const double final_energy = kinetic_energy(cpu) + spring_energy(cpu, contact_params.normal_stiffness);
  expect_near(final_energy, initial_energy, 2.0e-5, "CPU reference total energy");
  if (!(result.velocity_x[0] < -0.99 && result.velocity_x[1] > 0.99)) {
    std::fprintf(stderr, "collision did not reverse velocities as expected\n");
    return 1;
  }

  std::printf("GPU_DEM Hooke two-particle collision test passed on device %d (%s)\n",
