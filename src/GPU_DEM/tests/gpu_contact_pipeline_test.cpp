/* ----------------------------------------------------------------------
   Standalone GPU_DEM Hookean normal contact-force regression test.
------------------------------------------------------------------------- */
...
int main()
{
  ...
  contact_params.normal_stiffness = 200.0;
  ...
  const double expected_force = contact_params.normal_stiffness * 0.25;
  ...
  expect_near(result.force_x[0] + result.force_x[1] + result.force_x[2], 0.0, 1.0e-12,
              "net force x");

  std::printf("GPU_DEM Hooke normal contact-force test passed on device %d (%s)\n",
